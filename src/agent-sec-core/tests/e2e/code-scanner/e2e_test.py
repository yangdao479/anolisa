...
# ---------------------------------------------------------------------------
# Reuse unit-test conftest.py cases (zero duplication)
# ---------------------------------------------------------------------------
...
def _load_conftest_cases():
    """Dynamically load conftest.py from the unit-test directory.

    The directory name ``unit-test`` contains a hyphen which is illegal as
    a Python module name, so we use importlib to load it by file path.
    """
    spec = importlib.util.spec_from_file_location("conftest_cases", _CONFTEST_PATH)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


_cases = _load_conftest_cases()
...
@pytest.mark.parametrize(
    "scan_case",
    _cases.SCAN_TEST_CASES,
    ids=[_make_parametrize_id(tc) for tc in _cases.SCAN_TEST_CASES],
)
def test_all_rules_via_cli(scan_case: tuple) -> None:
    ...
    result = _parse_result(_run_scan(code, language=language.value))
