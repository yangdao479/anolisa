...
# OpenClaw tool_name -> (field in tool_input that holds the code, default language)
# Only `exec` (alias `bash`) can execute local shell commands.
# `code_execution` runs in a remote xAI sandbox and is not hooked.
TOOL_EXTRACTORS: Dict[str, Tuple[str, Language]] = {
    "exec": ("command", Language.BASH),
}
...
def extract_code_and_language(
    ...
    """Extract scannable code and its language from an OpenClaw tool call.
