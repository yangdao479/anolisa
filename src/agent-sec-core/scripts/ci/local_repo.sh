...
COMPONENT_INDEX="$V1/components-v2.toml"
TEMP_COMPONENT_INDEX="$V1/.components-v2.toml.tmp.$$"
cleanup() {
    rm -f "$TEMP_INDEX" "$TEMP_COMPONENT_INDEX"
}
trap cleanup EXIT
...
# A `--repo` override makes the named repository the identity authority for
# that invocation, so install validates the component name against this
# repository's `components-v2.toml` before it fetches anything. Without the
# file the client cannot validate the name and refuses the install, even
# though the distribution index above resolves. Only identity is declared
# here: with no backend row the raw backend derives the artifact name from
# the component, which is how the entry above is published.
cat > "$TEMP_COMPONENT_INDEX" <<EOF
# Generated by scripts/ci/local_repo.sh — do not publish.
schema_version = 2
publisher = "$PUBLISHER"
...
[[components]]
name = "$COMPONENT"
targets = [{ os = "$TARGET_OS", arch = "$TARGET_ARCH" }]
EOF
chmod 0644 "$TEMP_COMPONENT_INDEX"
mv -f "$TEMP_COMPONENT_INDEX" "$COMPONENT_INDEX"
